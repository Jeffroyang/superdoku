...
ConstraintSolver &ConstraintSolver::operator=(const Solver &other)
{
...
ConstraintSolver &ConstraintSolver::operator=(Solver &&other)
{
    Solver::operator=(other);
    init_board();
    return *this;
...
void ConstraintSolver::init_board()
{
...
void ConstraintSolver::populate_grid()
{
    for (int row = 0; row < GRID_SIZE; row++)
    {
        ...
        {
            if (board[row][col].size() == 1)
                game->update(row, col, (board[row][col][0] + '0'));
        }
    }
}
...
void ConstraintSolver::solve(bool printGame)
{
    if (printGame)
    {
